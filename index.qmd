...
p_forest = (
    ggplot(beta_summary.to_pandas(), aes("groups", "beta"))
    + geom_pointrange(aes(ymin="beta_lower", ymax="beta_upper"), size=0.8)
...
p_pred = (
    ...
    + geom_ribbon(
        aes(ymin="mu_lower", ymax="mu_upper", fill=as_discrete("group")), alpha=0.25
    )
    + geom_line(aes(y="mu", color=as_discrete("group")), size=0.8)
